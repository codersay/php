...
$.frameDialog = $.dialog.frameDialog = function(url, option){
	option = option || {};
	var iframe = "<iframe name='frameDialog' style='width:" + option.width + "px;height:" + option.height + "px;' scrolling='auto' frameborder='0' src='" + url + "'></iframe>";
	$.dialog({
		id : 'frameDialog',
		fixed : true,
		title : option.title,
		...
		content : iframe,
		padding : 0,
		beforeunload : option.closefun,
		initialize : option.initfun
	});
};
...
$.frameData = $.dialog.frameData = function( name, value ){
	var _data = '@ARTDIALOG.DATA';
	var top = docTop;
	var cache = top[_data] || {};
	top[_data] = cache;
	if ( value !== undefined ){
...
$.removeFrameData = $.dialog.removeFrameData = function(name){
	var _data = '@ARTDIALOG.DATA';
	var cache = docTop[_data];
