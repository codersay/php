 ...
 * @return void
 */
function widget($name, $action, $data=array(), $return=false) {
    $class      =   $name . 'Widget';
    require_cache(BASE_LIB_PATH . 'Widget/' . $class . '.class.php');
    if (!class_exists($class))
        throw_exception(L('_CLASS_NOT_EXIST_') . ':' . $class);
...
function X($name,$params=array(),$domain='model') {
    ...
    $class = $name.$domain;
    if(file_exists(CORE_PATH.$domain.'/'.$class.'.class.php')){
        tsload(CORE_PATH.$domain.'/'.$class.'.class.php');
    }else{
...
function tsload($filename) {
    ...
    if (!isset($_importFiles[$key])) {
        ...
        } elseif(file_exists(CORE_PATH.'/'.$filename.'.class.php')) {
            
            require_once CORE_PATH.'/'.$filename.'.class.php';
            $_importFiles[$key] = true;
        } else {
