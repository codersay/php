...
class Avatar{
	...
	function move_uploaded(){        //移动上传文件
		...
		if($result){
			//list($width_orig, $height_orig, $type) = getimagesize($this->saveDir."/".$file_newname);
			$info  = $this->getImageInfo($this->saveDir."/".$file_newname);
			$avatarw = 750;
			$avatarh = 750;
			$srcWidth  = $info['width']; //原图宽度
            $srcHeight = $info['height']; //原图高度
			if ($srcWidth>$avatarw||$srcHeight>$avatarw) {
		      	if ($srcHeight > $srcWidth){ // 竖图
		         	$newHeight = $avatarh;
		         	$newWidth = ceil($srcWidth / ($srcHeight / $avatarh ));
		      	}else if($srcHeight < $srcWidth){ // 横图
		         	$newHeight = ceil($srcHeight / ($srcWidth / $avatarw ));
		         	$newWidth = $avatarw;
		      	}
            	...
            	$srcImg     = $createFun($this->saveDir."/".$file_newname);
	            ...
	            if(function_exists("ImageCopyResampled"))
	                ...
	                imagecopyresized($thumbImg, $srcImg, 0, 0, 0, 0, $newWidth, $newHeight,  $srcWidth,$srcHeight);
	            ...
	            if( $type == 'jpg' || $type == 'jpeg' ){
	            	$imageFun($thumbImg,$this->saveDir."/".$file_newname,100);
	            }else{
	            	$imageFun($thumbImg,$this->saveDir."/".$file_newname);
	            }
			}
			$success['savename'] = $file_newname;
			$success['savepath'] = $this->saveDir."/";
			$success['size'] = $_FILES[$this->inputName]['size'];
			$success['name'] = $_FILES[$this->inputName]['name'];
			$success['type'] = $_FILES[$this->inputName]['type'];
			return array('success'=>$success,'status'=>1);
		}else{
			return array('error'=>$this->chk_err($_FILES[$this->inputName]['error']),'status'=>0);
		}
